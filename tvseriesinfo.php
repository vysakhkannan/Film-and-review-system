...
$query_str = "SELECT * FROM tvseries WHERE title = '" . $title . "'";
$query = mysqli_query($conn,"SELECT AVG(reviewrating) as AVGRATE from tvseriesreview WHERE tvseriestitle = '" . $title . "'");
...
<div class="row">
    ...
    <form action="tvserieswishlistinsert.php" method="post">
      <input type="hidden" name ="tit"value ="<?php echo $result_row['title']?>">
      <input type="hidden" name ="cov"value ="<?php echo $result_row['cover']?>">
      <button name="wishlist" type="submit" onclick= "location.href = ''" class="btn btn1">ADD TO WISHLIST</button></th>

</form>

    <form id="rev" action="reviewtvseries.php" method="post">
    <table >
<tr>
<td><h3 style="color:white">REVIEW US HERE:</h3></td>
<td><button value="<?php echo $result_row['title'] ?>" name="tit" onclick="document.getElementById('id01').style.display='block'" style="width:auto;">ADD REVIEW</button></td>
</tr>
...
<section>
<?php
$sql = "SELECT username, reviewrating, reviewcontent,time FROM tvseriesreview WHERE tvseriestitle='" . $title . "'";
...
<tr>
    ...
    <td><button   onclick= "location.href = 'reporttvseries.php?time=<?php echo $r['time']; ?>'">REPORT</button></td>
...
<center><p> <?php echo "No Reviews!!!!!!!!"; ?></p></center>
